...
<body>
	...
	<div id="collapseTwo" class="accordion-body collapse">
		<div class="accordion-inner">
			<form:form id="searchForm" modelAttribute="shelves" onkeypress="onEnterSearch(event,'shelvesTable')" class="form form-horizontal well clearfix">
			 <div class="col-xs-12 col-sm-6 col-md-4">
				<label class="label-item single-overflow pull-left" title="货架条码：">货架条码：</label>
			 ...
			 <div class="col-xs-12 col-sm-6 col-md-4">
				...
				<sys:gridselect url="${ctx}/base/supplier/data" id="supplier" name="supplier.id" value="${shelves.supplier.id}" labelName="supplier.name" labelValue="${shelves.supplier.name}"
								isMultiSelected="false" title="选择供应商" cssClass="form-control required" fieldLabels="供应商编码|供应商名称" fieldKeys="number|name" searchLabels="供应商编码|供应商名称" searchKeys="number|name" ></sys:gridselect>
			</div>
			 <div class="col-xs-12 col-sm-6 col-md-4">
				<label class="label-item single-overflow pull-left" title="是否绑定条码：">是否绑定条码：</label>
				<form:select path="isBind"  class="form-control m-b">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('shelves_is_bind')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
			 <div class="col-xs-12 col-sm-6 col-md-4">
				<label class="label-item single-overflow pull-left" title="货架类型：">货架类型：</label>
	...
	<!-- 工具栏 -->
	<div id="toolbar">
			<shiro:hasPermission name="base:shelves:add">
				<a id="add" class="btn btn-primary" onclick="add()"><i class="glyphicon glyphicon-plus"></i> 新建</a>
			</shiro:hasPermission>
			<shiro:hasPermission name="base:shelves:edit">
			    <button id="edit" class="btn btn-success" disabled onclick="edit()">
	            	<i class="glyphicon glyphicon-edit"></i> 修改
	        	</button>
			</shiro:hasPermission>
			<shiro:hasPermission name="base:shelves:del">
				<button id="remove" class="btn btn-danger" disabled onclick="deleteAll()">
	            	<i class="glyphicon glyphicon-remove"></i> 删除
	        	</button>
			</shiro:hasPermission>
			<shiro:hasPermission name="base:shelves:import">
				<button id="btnImport" class="btn btn-info"><i class="fa fa-folder-open-o"></i> 导入</button>
				<div id="importBox" class="hide">
						<form id="importForm" action="${ctx}/base/shelves/import" method="post" enctype="multipart/form-data"
			...
			<shiro:hasPermission name="base:kindClothing:export">
				<table:exportExcel url="${ctx}/base/shelves/export"></table:exportExcel><!-- 导出按钮 -->
			</shiro:hasPermission>
