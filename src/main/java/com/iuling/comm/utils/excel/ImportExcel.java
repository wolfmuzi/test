 ...
 */
package com.iuling.comm.utils.excel;
...
//							}
					...
					if (os[1] instanceof Field){
						...
						if (ef.value().contains(".") && valType != Area.class && valType != Office.class && valType != User.class){
							Object obj = Reflections.invokeMethodByName(e,mthodName,new Object[] {});
							if (obj == null){
								obj = ef.fieldType().newInstance();
							}
							String fieldName = ef.value().substring(ef.value().indexOf(".") + 1,ef.value().length());
							Reflections.invokeSetter(obj,fieldName,val);
							if("id".equalsIgnoreCase(fieldName) && StringUtils.isNotBlank((String)val)){
								Reflections.invokeMethod(e, "setIsNewRecord", new Class[] {boolean.class}, new Object[] {true});
							}
							val = obj;
							fieldName = ef.value().substring(0,ef.value().indexOf("."));
							if (!mthodName.replace("get","").equalsIgnoreCase(fieldName)){
								mthodName = "set" + fieldName.substring(0,1).toUpperCase() + fieldName.substring(1);
							}
						}
