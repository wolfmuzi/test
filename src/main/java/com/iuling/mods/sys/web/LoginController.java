...
import com.iuling.comm.utils.CookieUtils;
import com.iuling.comm.utils.IdGen;
import com.iuling.comm.utils.StringUtils;
import com.iuling.core.security.shiro.session.SessionDAO;
import com.iuling.core.servlet.ValidateCodeServlet;
...
import com.iuling.mods.sys.security.FormAuthenticationFilter;
...
public class LoginController extends BaseController{
	...
	public String login(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(principal != null && !principal.isMobileLogin()){
		...
		}else if (WebUtils.isTrue(request, FormAuthenticationFilter.DEFAULT_MOBILE_PARAM)){
			// 如果是手机登录，则返回JSON字符串
			//如果是手机登录记录设备使用记录
			...
			j.put("mobileLogin", principal.isMobileLogin());
		...
		return "mods/sys/login/sysLogin";
	}
	   ...
	   // 如果是手机客户端退出跳转到login，则返回JSON字符串
			...
			if(	ajax!=null){
				...
				if(equipments != null && equipments.size() > 0){
					...
					List<EquipmentUse> uses = equipmentUseService.findList(equipmentUse);
					if(uses != null && uses.size() > 0){
						EquipmentUse use = uses.get(0);
						use.setEndTime(new Date());
						equipmentUseService.save(use);
					}
				}
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		if (principal.isMobileLogin()){
			...
			if (request.getParameter("index") != null){
				return "mods/sys/login/sysIndex";
			}
			return "redirect:" + adminPath + "/login";
		}
		
		if(UserUtils.getMenuList().size() == 0){
			return "mods/sys/login/noAuth";
		}else{
			return "mods/sys/login/sysIndex";
		}
